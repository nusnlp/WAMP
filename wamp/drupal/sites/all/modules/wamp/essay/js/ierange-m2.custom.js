...
if (document.createRange) return; // browsers that can already create Ranges do not need IERange
	...
	convertToDOMRange: function (textRange, document) {
		function adoptBoundary(domRange, textRange, bStart) {
			...
			if (cursor.compareEndPoints(bStart ? 'StartToStart' : 'StartToEnd', textRange) == -1 && cursorNode.nextSibling) {
				...
				// PJH: remove newlines (\n) from cursor.text, since IE has a bug that
				// condenses all \n chars into a single space when accessing the text of 
				// a text node in <pre>
				// (see http://code.google.com/p/ierange/issues/detail?id=4 for details)
				// Also, whitespaces are removed/kept at "compile time" (before document.ready),
				// so whitespace in <pre> is kept, BUT the text in text nodes within <pre> (i.e. node.data) is condensed
				// making it impossible to access the uncondensed text directly, without using Selections
				var selectionText = cursor.text, selectionOffset = 0, nodeText = cursorNode.nextSibling.data, nodeOffset = 0;
				for (selectionOffset=0; selectionOffset<selectionText.length && nodeOffset<nodeText.length; selectionOffset++) {
					if (selectionText.charAt(selectionOffset) == nodeText.charAt(nodeOffset)) {
						nodeOffset++;
					}
				}
				domRange[bStart ? 'setStart' : 'setEnd'](cursorNode.nextSibling, nodeOffset);
				/* //Original code
	 ...
	 ,
	// PJH: TextRangeUtils.convertFromDOMRange() was buggy, but I've fixed it, 
	// so it, and all window.getSelection().* stuff should work now! Yippee! Finally! 20/07/2009
	convertFromDOMRange: function (domRange) {
		function getTextRangePoint(container, offset) {
			...
			var cursorNode = domRange._document.createElement('b');
			// PJH: Issue 5 fix
			if (anchorNode)
				anchorParent.insertBefore(cursorNode, anchorNode);
			else
				anchorParent.insertBefore(cursorNode);
			//anchorParent.insertBefore(cursorNode, anchorNode);
			var cursor = domRange._document.body.createTextRange();
			cursor.moveToElementText(cursorNode);
			cursorNode.parentNode.removeChild(cursorNode);
			// move range
			textRange.setEndPoint('StartToEnd', cursor);
		...
		var textRange = domRange._document.body.createTextRange();
		textRange.setEndPoint('StartToEnd', getTextRangePoint(domRange.startContainer, domRange.startOffset));
		textRange.setEndPoint('EndToEnd', getTextRangePoint(domRange.endContainer, domRange.endOffset));
		return textRange;
	}
...
//[TODO] collapse if start is after end, end is before start
	...
	insertNode: function (newNode) {
		...
		// PJH: Patch for inserting after the last element (when this.startOffset == this.startContainer.childNodes.length)
		} else if (this.startContainer.childNodes.length > this.startOffset) {
	...
	compareBoundaryPoints: function (how, sourceRange) {
		...
		// PJH: Patch for broken compare
		// reference implementation based on W3C's description of how Range.compareBoundaryPoints() works
		var TEXT_NODE = 3, ELEMENT_NODE = 1;
		
		// In the first case the boundary-points have the same container. 
		// A is before B if its offset is less than the offset of B, A is equal to B if its offset is equal to the offset of B, and A is after B if its offset is greater than the offset of B.
		...
		// If the first case does not apply (i.e. containerB != containerA),
		// make containerA and/or containerB refer to their parent elements if they are text nodes,
		// since (IE's) DOM functions like .contains() used in later cases only works with elements (not text nodes).
		if (containerA.nodeType == TEXT_NODE && containerA != containerB) {
			var siblings = containerA.parentNode.childNodes;
			for (var i=0; i<siblings.length; i++) {
				if (siblings[i] == containerA) {
					offsetA = i;
					break;
				}
			}
		...
		if (containerB.nodeType == TEXT_NODE && containerA != containerB) {
			var siblings = containerB.parentNode.childNodes;
			for (var i=0; i<siblings.length; i++) {
				if (siblings[i] == containerB) {
					offsetB = i;
					break;
				}
			}
		...
		// In the second case a child node C of the container of A is an ancestor container of B. 
		// In this case, A is before B if the offset of A is less than or equal to the index of the child node C and A is after B otherwise.
		if (containerA.contains(containerB)) {
			for (var c=0; c<containerA.childNodes.length; c++) {
				if (containerA.childNodes[c].nodeType == ELEMENT_NODE && containerA.childNodes[c].contains(containerB)) {
					if (offsetA <= c) 
						return -1;
					else 
						return 1;
		...
		// In the third case a child node C of the container of B is an ancestor container of A. 
		// In this case, A is before B if the index of the child node C is less than the offset of B and A is after B otherwise.
		if (containerB.contains(containerA)) {
			for (c=0; c<containerB.childNodes.length; c++) {
				if (containerB.childNodes[c].nodeType == ELEMENT_NODE && containerB.childNodes[c].contains(containerA)) {
					if (c < offsetB)
					//if (offsetB >= c)		// BUG OUT!!! (offsetB >= c) =/= (c < offsetB)?!?
						return -1;
					else 
						return 1;
				}
		...
		// In the fourth case, none of three other cases hold: the containers of A and B are siblings or descendants of sibling nodes. 
		// In this case, A is before B if the container of A is before the container of B in a pre-order traversal of the Ranges' context tree and A is after B otherwise.
		var ancestor = containerA.parentNode;
		while (ancestor.contains(containerB) == false) {
		...
		for (var i=0; i<ancestor.childNodes.length; i++) {
			...
			if (child.contains(containerA))
				indexA = i;
			if (child.contains(containerB))
				indexB = i;
		...
		/* Original IERange-m2's buggy code:
		// compare
...
//[TODO] Releases Range from use to improve performance. 
	...
	// This returns Selection.toString(), rather than Range.toString(), which IS DIFFERENT
	// e.g. in Firefox 3, Windows XP, for the |<p>Hello</p><p>World</p>|, 
	// the Range.toString() == 'HelloWorld', BUT Selection.toString() == 'Hello\r\n\r\nWorld' 
	// (i.e. Selection.toString() includes structure, while Range.toString() should not)!
	toString: function () {
		return new Error('Do NOT use this, since the behaviour is incorrect! See comments in code for details.');
...
function DOMSelection(document) {
	...
	var selection = this;
	this.prevFunc = document.onselectionchange;
	document.onselectionchange = function (event) { selection._selectionChangeHandler(); };
}
...
DOMSelection.prototype = {
	...
	removeAllRanges: function () {
		...
		this._selectionChangeHandler(); // PJH: it seems the onselectionchange event does not fire when the selction is emptied
	},
