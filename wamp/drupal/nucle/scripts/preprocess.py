...
class PreProcessor:
    ...
    def tokenizeCorrectionStr(self, correctionStr, wdstart, wdend, words):
        ...
        for sent in sentList:
            ...
            # The section below is added to prevent double quotes from being tokenized into two single quotes.
            tokenized_sent = [word.replace("``", '"') for word in tokenized_sent]
            tokenized_sent = [word.replace("''", '"') for word in tokenized_sent]
            pseudoWordsList += tokenized_sent
